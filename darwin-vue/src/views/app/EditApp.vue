...
const submit = async formEl => {
  const successHandle = () => ElNotification.success('编辑应用成功')
  const failHandle = () => ElNotification.success('编辑应用失败')
  if (!await executeAsyncRequest(asyncUpdateApp, appForm, successHandle, failHandle, formEl)) return
  open.value = false
}
  ...
  <el-dialog v-model="open" @close="emits('close')" width="680" align-center show-close>
    <el-space direction="vertical" :size="20" :fill="true" class="w100">
      <el-page-header @back="open = false">
        ...
        <template #content>
          <span class="text-large font-600 mr-3">编辑应用</span>
        </template>
      </el-page-header>
