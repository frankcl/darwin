...
public class MultiQueueMonitor extends ExecuteRunner {
    ...
    public static final String NAME = "MultiQueueMonitor";
    ...
    public MultiQueueMonitor(long executeTimeIntervalMs, long expiredTimeIntervalMs) {
    ...
    private void sweepExpiredJobs() {
        ...
        List<Job> jobs = jobService.getRunningJobs(before, BATCH_GET_SIZE);
        int sweepRecordCount = 0, sweepJobCount = 0;
        for (Job job : jobs) {
            List<URLRecord> expiredRecords = urlService.getJobExpiredRecords(job.jobId, before, BATCH_GET_SIZE);
