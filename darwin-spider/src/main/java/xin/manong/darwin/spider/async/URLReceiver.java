...
public class URLReceiver implements MessageListener {
    ...
    @Resource(name = "textRecordQueue")
    protected BlockingQueue<SpiderRecord> textRecordQueue;
    @Resource(name = "resourceRecordQueue")
    protected BlockingQueue<SpiderRecord> resourceRecordQueue;
    ...
    public Action consume(Message message, ConsumeContext consumeContext) {
        ...
        try {
            ...
            URLRecord record = JSON.parseObject(new String(body,
            ...
            SpiderRecord spiderRecord = new SpiderRecord(context, record);
            if (record.category == null || record.category == Constants.CONTENT_CATEGORY_TEXT ||
                    record.category == Constants.CONTENT_CATEGORY_LIST) textRecordQueue.put(spiderRecord);
            else resourceRecordQueue.put(spiderRecord);
            return Action.CommitMessage;
        } catch (Throwable t) {
